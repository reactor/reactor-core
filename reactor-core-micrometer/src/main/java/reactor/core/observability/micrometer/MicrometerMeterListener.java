...
import static reactor.core.observability.micrometer.PublisherMeters.CommonTags.*;
import static reactor.core.observability.micrometer.PublisherMeters.TerminationTags.*;
...
final class MicrometerMeterListener<T> implements SignalListener<T> {
	...
	MicrometerMeterListener(MicrometerMeterListenerConfiguration configuration) {
		...
		if (configuration.isMono) {
			...
			this.onNextIntervalTimer = Timer.builder(PublisherMeters.ON_NEXT_DELAY.getName(configuration.sequenceName))
			...
			if (!Micrometer.DEFAULT_METER_PREFIX.equals(configuration.sequenceName)) {
				this.requestedCounter = DistributionSummary.builder(PublisherMeters.REQUESTED.getName(configuration.sequenceName))
					.tags(configuration.commonTags)
					.register(configuration.registry);
			}
			else {
	...
	static void recordCancel(String name, Tags commonTags, MeterRegistry registry, Timer.Sample flowDuration) {
		Timer timer = Timer.builder(PublisherMeters.FLOW_DURATION.getName(name))
	...
	static void recordMalformed(String name, Tags commonTags, MeterRegistry registry) {
		registry.counter(PublisherMeters.MALFORMED.getName(name), commonTags)
			.increment();
	}
	...
	static void recordOnError(String name, Tags commonTags, MeterRegistry registry, Timer.Sample flowDuration,
		...
		Timer timer = Timer.builder(PublisherMeters.FLOW_DURATION.getName(name))
			...
			.tag(PublisherMeters.TerminationTags.EXCEPTION.asString(),
				e.getClass().getName())
			.description(
	...
	static void recordOnSubscribe(String name, Tags commonTags, MeterRegistry registry) {
		Counter.builder(PublisherMeters.SUBSCRIBED.getName(name))
			.tags(commonTags)
			.register(registry)
			.increment();
	}
