...
<2> We use the `tap` operator with the `observation` utility, making an anonymous `observation` available to upstream operators - in this case, the `listenToEvents()` chain.
...
----
<1> The `Observation` for this pipeline will be identified with the "events" prefix.
<2> We use the `tap` operator with the `observation` utility, making a custom `observation` available to upstream operators - in this case, the `listenToEvents()` chain.
<3> A registry must be provided into which to publish the observation results. Note this is an `ObservationRegistry`.
