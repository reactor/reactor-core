...
Consumer<SynchronousSink<Integer>> mySink = sink -> {
...
Flux<Integer> transientFlux = Flux.<Integer>generate(mySink)
...
<4> In other cases we emit an `onError`. That's 2 out of 3 times, so the flux bursts 2 `onError` interrupted by 1 `onNext`.
...
exception in an error-producing `Flux`, as follows: `return Flux.error(checkedException)`
(the sequence also terminates).
. Catch the exception, wrap it into an _unchecked_ exception, and then throw it
(interrupting the sequence). The `Exceptions` utility class can help you with that.
