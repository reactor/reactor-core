...
As we saw in section <<context>>, `ThreadLocal` doesn't make much sense in Reactive Streams, where several streams could execute steps on a common thread.
...
One recurring use of `ThreadLocal` in java is for logging frameworks to store variables that will be used in their logging pattern, generally by way of a `Map` called the MDC ("Mapped Diagnostic Context").

Unfortunately, this pattern is hard to opt out of and hard to apply transparently from a reactive library perspective.
This entry of the FAQ offers a possible intermediate solution when one wants to perform log statements in relation to events in the stream (`onNext`, `onError`, `onComplete`).
...
Nothing much has changed except for the fact that we check the `Signal` is effectively an `onError`, and we provide said error (a `Throwable`) to the log statement lambda.
