...
data through it while its `Subscriber` has not yet requested date, it will buffer all of
the data.
...
* a limited history (`create(int)`), unbounded history (`create()`).
* A time-based replay window (`createTimeout(Duration)`).
* A combination of history size and time window (`createSizeOrTimeout(int, Duration)`).
