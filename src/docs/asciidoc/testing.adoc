...
you want to manipulate a test-specific state or context. For example, you might use
...
method. The latter takes a value from the `TestPublisher.Violation` enum. The values
define which parts of the specification the publisher can overlook. These enum values
include:
