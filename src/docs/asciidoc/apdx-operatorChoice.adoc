...
** ...but lazily build the `Throwable`: `Mono#error(Supplier<Throwable>)`
...
*** ...in order to materialize each source value's index: `Flux#indexed`
...
** ...lazily: `Mono#error(Supplier<Throwable>)`
