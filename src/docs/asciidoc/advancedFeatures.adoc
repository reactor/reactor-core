...
WARNING: Grouping is best suited for when you have a low cardinality of groups.
...
constraints combine and lead to hangs, like when you have a high cardinality and the
...
opened. So if `skip < maxSize`, a new window will open before the previous one closes and
...
With the reverse configuration (skip > maxSize), some elements from the source would be
