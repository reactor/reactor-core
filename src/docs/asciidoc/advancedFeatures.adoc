...
TIP: The global hook also has a `Flux`/`Mono`-local version. By using `doOnDiscard`, specifying a `Class` to take into account, one can set up discard hook that are specific to a given sequence.
These hooks are also additive, but only visible and used by operators upstream of the call to `onDiscard(Class, Consumer)`.
