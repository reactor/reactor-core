...
final class ParallelScheduler implements Scheduler, Supplier<ScheduledExecutorService>,
    ...
    void init(int n) {
        ...
        for (int i = 0; i < n; i++) {
            a[i] = Schedulers.decorateExecutorService(Schedulers.PARALLEL, this.toString(), this.get());
        }
    ...
    public void start() {
        ...
        for (;;) {
            ...
            if (b == null) {
                ...
                for (int i = 0; i < n; i++) {
                    b[i] = Schedulers.decorateExecutorService(Schedulers.PARALLEL, this.toString(), this.get());
                }
            }
