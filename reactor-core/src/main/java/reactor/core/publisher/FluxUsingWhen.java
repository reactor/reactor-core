...
final class FluxUsingWhen<T, S> extends Flux<T> implements SourceProducer<T> {
		...
		if (resourceSupplier instanceof Callable) {
			try {
				...
				if (resource == null) {
					...
					Tuple2<Publisher<? extends T>, UsingWhenSubscriber<? super T, S>>
							tuple = deriveFluxAndSubscriber(resource,
							actual,
							resourceClosure,
							asyncComplete,
							asyncError,
	...
	private static <RESOURCE, T> Tuple2<Publisher<? extends T>, UsingWhenSubscriber<? super T, RESOURCE>> deriveFluxAndSubscriber(
			RESOURCE resource,
		...
		if (actual instanceof ConditionalSubscriber) {
			...
			UsingWhenSubscriber<? super T, RESOURCE> uwcs =
					new UsingWhenConditionalSubscriber<>(conditionalActual,
							resource,
							asyncComplete,
							asyncError,
							asyncCancel,
			...
			UsingWhenSubscriber<? super T, RESOURCE> uws = new UsingWhenSubscriber<>(actual,
					resource,
					asyncComplete,
					asyncError,
					asyncCancel,
	...
	static class ResourceSubscriber<S, T> extends DeferredSubscription
		...
		public void onNext(S resource) {
			...
			final Tuple2<Publisher<? extends T>, UsingWhenSubscriber<? super T, S>>
					tuple = deriveFluxAndSubscriber(resource,
					this.actual,
