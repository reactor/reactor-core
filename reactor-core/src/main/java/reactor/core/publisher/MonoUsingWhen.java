...
final class MonoUsingWhen<T, S> extends Mono<T> implements SourceProducer<T> {
		...
		if (resourceSupplier instanceof Callable) {
			try {
				...
				if (resource == null) {
					...
					final Tuple2<Mono<? extends T>, UsingWhenSubscriber<? super T, S>>
							tuple = deriverMonoAndSubscriber(resource,
							actual,
							resourceClosure,
							asyncComplete,
							asyncError,
					...
					tuple.getT1().subscribe(tuple.getT2());
				}
			}
			catch (Throwable e) {
	...
	private static <RESOURCE, T> Tuple2<Mono<? extends T>, UsingWhenSubscriber<? super T, RESOURCE>> deriverMonoAndSubscriber(
			RESOURCE resource,
		...
		if (actual instanceof ConditionalSubscriber) {
			...
			FluxUsingWhen.UsingWhenConditionalSubscriber<? super T, RESOURCE> uwcs =
					new FluxUsingWhen.UsingWhenConditionalSubscriber<>(conditionalActual,
							resource,
							asyncComplete,
							asyncError,
							asyncCancel,
							arbiter);
			return Tuples.of(p, uwcs);
		}
		else {
			final UsingWhenSubscriber<? super T, RESOURCE> uws =
					new UsingWhenSubscriber<>(actual,
							resource,
							asyncComplete,
							asyncError,
							asyncCancel,
							arbiter);
			return Tuples.of(p, uws);
		}
	}

	//needed to correctly call deriverMonoAndSubscriber with Mono.from conversions
	static class ResourceSubscriber<S, T> extends DeferredSubscription implements InnerConsumer<S> {
		...
		public void onNext(S resource) {
			...
			final Tuple2<Mono<? extends T>, UsingWhenSubscriber<? super T, S>> tuple =
					deriverMonoAndSubscriber(resource,
							this.actual,
							this.resourceClosure,
							this.asyncComplete,
							this.asyncError,
							this.asyncCancel,
							this);

			this.closureSubscriber = tuple.getT2();
			tuple.getT1()
			     .subscribe(closureSubscriber);
